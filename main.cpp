...
/*#if 0
    ...
    private: // Member Classes
        ...
        public: // Public Static Functions
            static std::string stateToKey(const State& state) {
                std::string key;
                int i=0,j;
        ...
        public: //Constructors, Destructors, & Operators
            ...
            explicit EndMovesTreeNode(State state, std::shared_ptr<EndMovesTreeNode> parent= nullptr, int empty_row= -1, int empty_col= -1, int depth= -1)
            ...
            EndMovesTreeNode(EndMovesTreeNode&& other) noexcept {
    ...
    private: // Private data members
        ...
        State start_state, win_state;

    private: // Private Functions
        std::shared_ptr<EndMovesTreeNode> buildStateTree(const State& state, int& node_num,
    ...
    public: // Public Static Functions
        static State generateWinState(const State& state){
            vector<int> choices;
            State res;
        ...
        static State generateEndState(const int& size, const int& row_len = 3, const int& col_len = 2){
            State endstate;
        ...
        static State generateEndState(const vector<vector<std::shared_ptr<GridNode>>>& grid)  {
            std::State remaining(2);
    ...
    public: // Public functions
        [[nodiscard]] const State& getStartState() const{
            return static_cast<const State &>(start_state);
        }
        [[nodiscard]] const State& getWinState() const{
            return static_cast<const State &>(win_state);
        }
        [[nodiscard]] vector<int> generate_moveslist(State* state_ptr=nullptr) const{
    ...
    public: //Constructors, Destructors, & Operators
        explicit EndMovesTree(State start_state): node_count(0), start_state(std::move(start_state)){
...
/*
        ...
        _print_board();
        cout << _board.size() << "-by-" << _board.size() << " puzzle solved in: " << elapsed.count() << "ms\n\n";
*/
...
std::vector<int> slide_puzzle(const vector<vector<int>> &arr){
    ...
    SlidingPuzzleSolver sps = SlidingPuzzleSolver(arr, true);
    cout << sps << endl;
    auto res = sps.solve();
    //cout << sps << "\n" << endl;
    return res;
...
/*        j=0;
    ...
    if (size)
        ...
        cout << "Evaluated " << n << " puzzles of sizes: \n";
        std::for_each(SlidingPuzzleSolver::getEndvector<vector<int>>s().begin(), --SlidingPuzzleSolver::getEndvector<vector<int>>s().end(), [](const auto& endstate){
            cout << " - " << endstate.first << "-by-" << endstate.first << ",\n";
        });
        cout << " - " << (--SlidingPuzzleSolver::getEndvector<vector<int>>s().end())->first << "-by-" << (--SlidingPuzzleSolver::getEndvector<vector<int>>s().end())->first << "\n";
    }
